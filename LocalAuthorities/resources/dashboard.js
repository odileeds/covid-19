...
(function(root){
	...
	var datasources = {
		...
		'uk-historic':{
			...
			'preProcess': function(dat){
				var data,byid,max,i,name,d,d2,id,id2,v,prevd,ndays;
				data = dat.data;
				byid = {};
				max = 0;
				for(id in data){
					...
					if(id && id.indexOf('W06')!=0){
						...
						for(v = 0; v < data[id].v.length; v++){
							...
							if(t > 0){
								byid[id].days[d2] = {'cases':t,'percapita':0,'d':d.addDays(v)};
								if(t > max) max = t;
				...
				for(var code in byid){
					if(byid[code]){
						...
						for(d in byid[code].days){
							...
							if(!prevd){
								byid[code].days[d].daily = 0;
								prevd = d;
							}else{
								if(byid[code].days[d].cases > 0){
									if(byid[code].days[prevd]){
										ndays = Math.round((byid[code].days[d].d - byid[code].days[prevd].d)/86400000);
										byid[code].days[d].daily = (byid[code].days[d].cases - byid[code].days[prevd].cases)/ndays;
									}
									prevd = d;
								}
							}
	...
	function DashboardBuilder(opts){
		...
		var colours = ['c1-bg','c5-bg','c11-bg','c14-bg','c4-bg','c6-bg','c13-bg'];
		...
		this.updateAreas = function(){
				...
				this.typeahead = TypeAhead.init('#typeahead',{
					...
					'process': function(d){
						...
						if(!match){
							...
							// Trigger the callback
							_obj.trigger('changeareas',{'areas':_obj.qs.areas,'colours':colours});
						}
					},
					'blur': function(){
						// Trigger the callback
			...
			// Trigger the callback
			this.trigger('changeareas',{'areas':_obj.qs.areas,'colours':colours});
		...
		this.addToggle = function(id,update){
			...
			if(datasources['uk-historic'].data[id] && S('#toggle-'+id).length==0){
				...
				li.setAttribute('class',colours[S('#toggle-holder .toggles li').length % colours.length]);
