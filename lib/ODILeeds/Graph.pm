...
sub draw {
	my ($self, $props) = @_;
	my ($n,$r,$w,$h,@lines,$svg,@header,%headers,$c,@cols,@rows,@linestyles,$id,$i,$s,$series,$safeseries,$minx,$maxx,$miny,$maxy,$path,$x,$y,$xrange,$yrange,$xpos,$ypos,$t,@pos,$circles,%ticks,@a,@b,$left,$right,$top,$bottom,$style,$wkey,$hkey,$fs,$pd);
	...
	if($props->{'key'}){
		if($props->{'key'}->{'border'}){ $svg .= "\t.key rect { ".$props->{'key'}->{'border'}." }\n"; }
		if($props->{'key'}->{'text'}){ $svg .= "\t.key text { ".$props->{'key'}->{'text'}." }\n"; }
	}
	...
	for($s = 0; $s < $n; $s++){
		...
		$path = "";
		$id = ($self->{'series'}[$s]{'id'});
		$svg .= "<g ".($id ? " id=\"".($id)."\"" : "")." class=\"data-series data-series-$s\">";
		$circles = "";
		$style = "";
		for($i = 0; $i < @{$self->{'series'}[$s]{'data'}}; $i++){
			if($self->{'series'}[$s]{'data'}[$i]){
				...
				if($self->{'series'}[$s]{'point'} && $self->{'series'}[$s]{'point'} > 0){
					$circles .= "\t<circle cx=\"".sprintf("%0.2f",$xpos)."\" cy=\"".sprintf("%0.2f",$ypos)."\" data-y=\"".sprintf("%0.".($self->{'series'}[$s]{'pointprec'}||2)."f",$self->{'series'}[$s]{'data'}[$i]{'y'})."\" data-x=\"".$self->{'series'}[$s]{'data'}[$i]{'x'}."\" r=\"".$self->{'series'}[$s]{'point'}."\" fill=\"".($self->{'series'}[$s]{'colour'}||"#cc0935")."\">";
					if($self->{'series'}[$s]{'data'}[$i]{'title'}){
						$circles .= "<title>".($self->{'series'}[$s]{'data'}[$i]{'title'})."</title>";
					}else{
						$circles .= "<title>".($self->{'series'}[$s]{'data'}[$i]{'label'}||$self->{'series'}[$s]{'data'}[$i]{'x'}.": ".$self->{'series'}[$s]{'data'}[$i]{'y'})."</title>";
					}
					$circles .= "</circle>\n";
				}
			}
		...
		# Set linestyle
		$linestyles[$s] = " style=\"stroke:".($self->{'series'}[$s]{'colour'}||"black")."\"";
		if($self->{'series'}[$s]{'stroke-width'}){
			$linestyles[$s] .= " stroke-width=\"".$self->{'series'}[$s]{'stroke-width'}."\"";
		}
		if($self->{'series'}[$s]{'fill'}){ $linestyles[$s] .= " fill=\"".$self->{'series'}[$s]{'fill'}."\""; }
		if($self->{'series'}[$s]{'fill-opacity'}){ $linestyles[$s] .= " fill-opacity=\"".$self->{'series'}[$s]{'fill-opacity'}."\""; }
		if($self->{'series'}[$s]{'opacity'}){ $linestyles[$s] .= " opacity=\"".$self->{'series'}[$s]{'opacity'}."\""; }
		$linestyles[$s] .= " stroke-linecap=\"round\"";
		if($self->{'series'}[$s]{'stroke-dasharray'}){ $linestyles[$s] .= " stroke-dasharray=\"".$self->{'series'}[$s]{'stroke-dasharray'}."\""; }
		if($self->{'series'}[$s]{'stroke-dashoffset'}){ $linestyles[$s] .= " stroke-dashoffset=\"".$self->{'series'}[$s]{'stroke-dashoffset'}."\""; }

		$svg .= $linestyles[$s]."><title>".$safeseries."</title></path>\n";
	...
	if($props->{'key'}){
		$fs = ($props->{'key'}->{'font-size'}||16);
		$pd = ($props->{'key'}->{'padding'}||5);
		$wkey = ($props->{'key'}->{'width'}||100);
		$hkey = ($props->{'key'}->{'label'} ? 1:0)*$fs+(2*($props->{'key'}->{'padding'}||$pd));
		for($s = 0; $s < $n; $s++){
			if($self->{'series'}[$s]{'key'}){
				$hkey += $fs;
			}
		}
		$x = ($w-$right-$wkey-0.5);
		$y = ($top-0.5);
		$svg .= "<g class=\"key\">\n";
		$svg .= "\t<rect x=\"".$x."\" y=\"".$y."\" width=\"".$wkey."\" height=\"".($hkey)."\"></rect>";
		$y += $pd;
		$x += $pd;
		if($props->{'key'}->{'label'}){
			$svg .= "\t<text x=\"".$x."\" y=\"".$y."\">";
			$svg .= "\t\t<tspan>".safeXML($props->{'key'}->{'label'})."</tspan>\n";
			$svg .= "\t</text>\n";
			$y += $fs;
		}
		for($s = 0; $s < $n; $s++){
			...
			$id = ($self->{'series'}[$s]{'id'});
			$svg .= "\n\t<g ".($id ? " id=\"".($id)."-key\">" : "").">";
			$svg .= "\t<text x=\"".$x."\" y=\"".($y+($s)*$fs)."\">";
			$svg .= "\t\t<tspan dx=\"".($fs*2)."\" dy=\"0\">".safeXML($self->{'series'}[$s]{'title'}||"")."</tspan>\n";
			$svg .= "</text>";
			$svg .= "\n\t<path d=\"M".sprintf("%0.2f",$x).",".sprintf("%0.2f",$y+(0.5+$s)*$fs)." l ".($fs*1.5)." 0\" class=\"line\"";
			if($linestyles[$s]){
				$svg .= $linestyles[$s];
			}
			$svg .= " />\n";
			if($self->{'series'}[$s]{'point'} && $self->{'series'}[$s]{'point'} > 0){
				$svg .= "\t<circle cx=\"".sprintf("%0.2f",$x+$fs*0.75)."\" cy=\"".sprintf("%0.2f",$y+(0.5+$s)*$fs)."\" r=\"".$self->{'series'}[$s]{'point'}."\" fill=\"".($self->{'series'}[$s]{'colour'}||"#cc0935")."\" />";
			}
			$svg .= "\t</g>\n";
		}
		$svg .= "</g>\n";
			...
			if($a[1] >= 0 && $a[0] >= $conf->{'left'}){
				if($a[0] < $conf->{'width'}){
					if(($t == 0 && $props->{'line'} > 0) || $props->{'lines'}){
						$svg .= "\t<line x1=\"".sprintf("%0.2f",$a[0]-0.5)."\" y1=\"".sprintf("%0.2f",$a[1]-0.5)."\" x2=\"".sprintf("%0.2f",$b[0]-0.5)."\" y2=\"".sprintf("%0.2f",$b[1]-0.5)."\" data-left=\"".$conf->{'left'}."\"></line>\n";
					}
					if($props->{'ticks'}){
						$svg .= "\t<line class=\"tick\" x1=\"".sprintf("%0.2f",$a[0]-0.5)."\" y1=\"".sprintf("%0.2f",$a[1]-0.5)."\" x2=\"".sprintf("%0.2f",($a[0]-($axis eq "y" ? $tick : 0))-0.5)."\" y2=\"".sprintf("%0.2f",($a[1]+($axis eq "y" ? 0 : $tick))-0.5)."\"></line>\n";
					}
					$label = $ticks{'data-'.$t};
					if($ticks{'label-'.$t}){
						$label = $ticks{'label-'.$t};
